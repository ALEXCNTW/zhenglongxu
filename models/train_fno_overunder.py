...
# FNO overunder
class FNO1dOverUnder(nn.Module):
    ...
    def forward(self, x):
        x = x.squeeze(1)  # [B, 1, 3] → [B, 3]
        x = self.fc0(x)   # [B, 3] → [B, width]
        x = x.unsqueeze(-1)  # [B, width] → [B, width, 1]
        x1 = self.conv1(x)
        x2 = self.conv2(x1)
        x3 = self.conv3(x2)
        x4 = self.conv4(x3)
        x = x4 + self.w(x)
        x = x.permute(0, 2, 1)  # [B, 1, width]
...
# data loading
def load_data():
    data = np.load("data_preprocessing/processed_overunder.npz")
...
def train(model, optimizer, loss_fn, X_train, y_train, X_val, y_val, epochs=100):
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            pred = torch.sigmoid(val_out) > 0.5
            acc = accuracy_score(y_val.numpy(), pred.numpy())
        if epoch % 10 == 0 or epoch == epochs - 1:
            print(f"Epoch {epoch:03d} | Train Loss: {loss.item():.4f} | Val Acc: {acc:.4f}")
    torch.save(model.state_dict(), "models/fno_overunder.pth")
    print("Over-Under prediction model training completed, and the model has been saved.")

# start
if __name__ == "__main__":
    X_train, y_train, X_val, y_val = load_data()
    X_train, y_train = X_train.float().unsqueeze(1), y_train.float().unsqueeze(1)
    X_val, y_val = X_val.float().unsqueeze(1), y_val.float().unsqueeze(1)
    model = FNO1dOverUnder()
